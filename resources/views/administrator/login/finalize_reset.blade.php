...
<body>
    ...
    <div class="col-md-12">
        ...
        <div class="login-logo">
            ...
            <p class="login-box-msg">Finalize Reset Password</p>

            <form action="{{ url('/admin/complete_reset') }}" method="post">
                {{ csrf_field() }}
                <div class="form-group{{ $errors->has('email') ? ' has-error' : '' }} has-feedback">
                    <input type="text" name="activation_key" class="form-control" value="{{ old('email') }}" placeholder="Activation Key" required>
                ...
                <div class="form-group{{ $errors->has('email') ? ' has-error' : '' }} has-feedback">
                    <input type="password" name="new_password" class="form-control" value="{{ old('email') }}" placeholder="New Password" required>
                    <span class="glyphicon glyphicon-key form-control-feedback"></span>
                    @if ($errors->has('email'))
                    <span class="help-block">
                        <strong>{{ $errors->first('email') }}</strong>
                ...
                <div class="form-group{{ $errors->has('email') ? ' has-error' : '' }} has-feedback">
                    <input type="password" name="confirmation_password" class="form-control" value="{{ old('email') }}" placeholder="New Password" required>
                    <span class="glyphicon glyphicon-key form-control-feedback"></span>
                    @if ($errors->has('email'))
                    <span class="help-block">
                        <strong>{{ $errors->first('email') }}</strong>
