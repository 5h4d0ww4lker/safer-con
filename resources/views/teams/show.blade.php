@extends('master')

@section('content')

<div class="panel panel-default">
    <div class="panel-heading clearfix">

        <span class="pull-left">
            <h4 class="mt-5 mb-5">{{ isset($team->label) ? $team->label : 'Team' }}</h4>
        </span>

        <div class="pull-right">

            <form method="POST" action="{!! route('teams.team.destroy', $team->id) !!}" accept-charset="UTF-8">
            <input name="_method" value="DELETE" type="hidden">
            {{ csrf_field() }}
                <div class="btn-group btn-group-sm" role="group">
                    <a href="{{ route('teams.team.index') }}" class="btn btn-primary" title="Show All Team">
                        <span class="glyphicon glyphicon-th-list" aria-hidden="true"></span>
                    </a>

                    <a href="{{ route('teams.team.create') }}" class="btn btn-success" title="Create New Team">
                        <span class="glyphicon glyphicon-plus" aria-hidden="true"></span>
                    </a>
                    
                    <a href="{{ route('teams.team.edit', $team->id ) }}" class="btn btn-primary" title="Edit Team">
                        <span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>
                    </a>

                    <button type="submit" class="btn btn-danger" title="Delete Team" onclick="return confirm(&quot;Click Ok to delete Team.?&quot;)">
                        <span class="glyphicon glyphicon-trash" aria-hidden="true"></span>
                    </button>
                </div>
            </form>

        </div>

    </div>

    <div class="panel-body">
        <dl class="dl-horizontal">
            <dt>Label</dt>
            <dd>{{ $team->label }}</dd>
            <dt>Description</dt>
            <dd>{{ $team->description }}</dd>
            <dt>Image</dt>
            <dd>{{ $team->image }}</dd>
            <dt>Status</dt>
            <dd>{{ $team->status }}</dd>
            <dt>Created By</dt>
            <dd>{{ optional($team->creator)->name }}</dd>
            <dt>Updated By</dt>
            <dd>{{ optional($team->updater)->name }}</dd>
            <dt>Deleted By</dt>
            <dd>{{ optional($team->deletedBy)->id }}</dd>
            <dt>Created At</dt>
            <dd>{{ $team->created_at }}</dd>
            <dt>Updated At</dt>
            <dd>{{ $team->updated_at }}</dd>
            <dt>Deleted At</dt>
            <dd>{{ $team->deleted_at }}</dd>

        </dl>

    </div>
</div>

@endsection